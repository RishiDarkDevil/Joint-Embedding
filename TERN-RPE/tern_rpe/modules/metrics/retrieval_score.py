...
#     USE_FAISS=False
USE_FAISS = False # THE ABOVE LINES COMMENTED TO TEST REMOVE THIS LINE AND UNCOMMENT ABOVE
...
class RetrievalScore():    
    def __init__(self, 
        ...
        if USE_FAISS:
            # res = faiss.StandardGpuResources()  # use a single GPU --> UNCOMMENT IF YOU HAVE GPU
            self.faiss_pool = faiss.IndexFlatIP(dimension)
            # self.faiss_pool = faiss.index_cpu_to_gpu(res, 0, self.faiss_pool) --> UNCOMMENT IF YOU HAVE GPU
        else:
            # Distance function
