...
def main(args, config):
    ...
    # res = faiss.StandardGpuResources()  # use a single GPU # UNCOMMENT ON GPU
    faiss_pool = faiss.IndexFlatIP(config.model['args']['d_embed'])
    # faiss_pool = faiss.index_cpu_to_gpu(res, 0, faiss_pool) # UNCOMMENT ON GPU
