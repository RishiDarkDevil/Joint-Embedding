...
class Trainer():
    ...
    def training_epoch(self):
        ...
        # self.trainloader.create_batches() --> SINCE, I AM USING DataLoader instead of BucketIterator
        # for i, raw_batch in enumerate(self.trainloader.batches): --> SAME REASON AS ABOVE
        i = 0 # Added for counting the batch number in DataLoader below
        ...
        for batch in self.trainloader:
            ...
            if self.use_accumulate:
                ...
                if self.scheduler is not None and not self.step_per_epoch:
                    ...
                    log_dict = {'Training/Learning rate': lr}
                    self.logging(log_dict, step=self.iters)
            i += 1 # ADDED BECAUSE CAN'T USE enumerate with DataLoader
                

            # torch.cuda.synchronize() --> UNCOMMENT ON GPU
